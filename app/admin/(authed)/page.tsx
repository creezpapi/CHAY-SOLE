        ...
        <div>
              <div className="flex items-center justify-between mb-6">
                      <div className="flex items-baseline gap-3">
                                <h1 className="text-2xl font-medium">Creatives</h1>h1>
                                <span className="text-sm text-rv-tab-inactive">{counts.all}</span>span>
                      </div>div>
                      <NewCreativeButton />
              </div>div>
              <StatusTabs counts={counts} activeStatus={status || 'all'} activePlatform={platform} activeProduct={product} />
              <div className="flex items-center gap-3 mt-4 mb-2">
                      <span className="text-xs text-rv-tab-inactive flex-shrink-0">Filter by product:</span>span>
                      <div className="flex-1 max-w-xs">
                                <ProductFilter products={allProducts || []} activeProduct={product} />
                      </div>div>
                {(platform || product || (status && status !== 'all')) && (
                    <Link href="/admin" className="text-xs text-rv-tab-inactive hover:text-black transition-colors flex-shrink-0">Clear all</Link>Link>
                      )}
              </div>div>
              <div className="mt-2">
                {creatives.length === 0 ? (
                    <div className="py-16 text-center text-rv-tab-inactive text-sm">No creatives match these filters.</div>div>
                  ) : (
                    <ul>
                                                    ...
                                                    <Link
                                                                      ...
                                                                      <div className="flex-1 min-w-0">
                                                                                          <div className="flex items-center gap-2">
                                                                                                                <p className="font-medium text-sm truncate">{creative.title}</p>p>
                                                                                            {creative.is_top_performer && (
                                                                                                <span className="flex-shrink-0 h-4 px-1.5 rounded-full bg-amber-100 text-amber-700 text-xs font-medium flex items-center">Top</span>span>
                                                                                                                )}
                                                                                            {creative.ad_code && (
                                                                                                <span className="flex-shrink-0 h-4 px-1.5 rounded-full bg-blue-100 text-blue-700 text-xs font-medium flex items-center">WL</span>span>
                                                                                                                )}
                                                                                            </div>div>
