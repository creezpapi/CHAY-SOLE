...
-- Seed first admin (update email as needed)
...
create index if not exists creatives_top_performer_idx on public.creatives(is_top_performer) where is_top_performer = true;
create index if not exists creatives_ad_code_idx on public.creatives(ad_code) where ad_code is not null;
create index if not exists creatives_drop_tag_idx on public.creatives(drop_tag) where drop_tag is not null;
...
drop trigger if exists trg_manual_products_updated on public.manual_products;
create trigger trg_manual_products_updated before update on public.manual_products
  for each row execute function public.set_updated_at();
...
drop policy if exists "Admins can manage manual_products" on public.manual_products;
create policy "Admins can manage manual_products" on public.manual_products for all using (public.is_admin()) with check (public.is_admin());
