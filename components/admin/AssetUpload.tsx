        ...
        {displayUrl ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => fileRef.current?.click()} disabled={isPending || !!uploadProgress}
              ...
              <button type="button" disabled={isPending || !!uploadProgress}
                onClick={() => { startTransition(async () => { await removeAsset(creative.id, creative.asset_path!); router.refresh(); }); }}
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {carouselImages.length > 0 && (
              <span className="text-xs text-rv-tab-inactive">({carouselImages.length})</span>
            )}
          </div>
          <button type="button" onClick={() => carouselFileRef.current?.click()}
          ...
          <div className="grid grid-cols-3 gap-2">
            {carouselImages.map((url, idx) => (
              <div key={idx} className="relative group aspect-square bg-rv-gray overflow-hidden">
                ...
                <span className="absolute bottom-1 left-1 text-white text-xs bg-black/50 px-1 rounded">{idx + 1}</span>
              </div>
            ))}
          </div>
        ) : (
