...
import { PLATFORMS, DROP_TAGS } from '@/lib/types';
...
export default function CreativeMetaForm({ creative }: { creative: Creative }) {
    ...
    const [dropTag, setDropTag] = useState<string>(creative.drop_tag || '');
  ...
  return (
        <div>
              <h2 className="text-base font-medium mb-4">Details</h2>h2>
              <form ref={formRef} action={handleSubmit} className="space-y-4">
                      <div>
                                <label className="block text-sm font-medium mb-1.5" htmlFor="title">Title</label>label>
                      ...
                      <div>
                                <label className="block text-sm font-medium mb-1.5" htmlFor="status">Status</label>label>
                                <select id="status" name="status" defaultValue={creative.status}
                                            ...
                                            <option value="ready_to_launch">Ready to Launch</option>option>
                                            <option value="active">Active</option>option>
                                            <option value="archived">Archived</option>option>
                                </select>select>
                      </div>div>
                      <div>
                                <label className="block text-sm font-medium mb-2">Drop Tag</label>label>
                                <div className="flex flex-wrap gap-2">
                                            <button type="button" onClick={() => setDropTag('')}
                                  ...
                                  {DROP_TAGS.map((tag) => (
                        <button key={tag} type="button" onClick={() => setDropTag(dropTag === tag ? '' : tag)}
                                          className={'h-8 px-3 rounded-full text-xs font-medium transition-all duration-250 active:scale-95 ' +
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {isTopPerformer && (
                      <span className="text-xs text-amber-600 font-medium">Tagged as DROP 1</span>span>
                                )}
                      </div>div>
                      <div>
                                <label className="block text-sm font-medium mb-2">Platforms</label>label>
                                <div className="flex flex-wrap gap-2">
                                  {PLATFORMS.map((p) => {
                      ...
                      <div>
                                <label className="block text-sm font-medium mb-1.5" htmlFor="notes">Notes</label>label>
                      ...
                      <div>
                                <label className="block text-sm font-medium mb-1.5" htmlFor="ad_copy">Ad Copy</label>label>
                      ...
                      <div className="pt-2">
                                <h3 className="text-sm font-semibold uppercase tracking-wider text-rv-tab-inactive mb-3">Whitelisting</h3>h3>
                                <div className="space-y-4 pl-3 border-l-2 border-rv-gray">
                                            <div>
                                                          <label className="block text-sm font-medium mb-1.5" htmlFor="post_link">
                                                                          Post Link <span className="text-rv-tab-inactive font-normal">(optional)</span>span>
                                                          </label>label>
                                            ...
                                            <div>
                                                          <label className="block text-sm font-medium mb-1.5" htmlFor="ad_code">
                                                                          Ad Code <span className="text-rv-tab-inactive font-normal">(optional)</span>span>
                                                          </label>label>
